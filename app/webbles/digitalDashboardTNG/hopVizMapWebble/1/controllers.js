//======================================================================================================================
// Controllers for hopVizMapWebble for Webble World v3.0 (2013)
...
//=======================================================================================
wblwrld3App.controller('hopVizMapWebbleCtrl', function($scope, $log, $timeout, Slot, Enum) {
	...
	try {
    		...
    		// if(targetField.name == 'Start Longitude') {
    		// 	dataName = dataSourceInfo.fieldName;
    		// } 
    		// if(targetField.name == 'Start Latitude') {
		// }
    		// if(targetField.name == 'End Longitude') {
		// }
    		// if(targetField.name == 'End Latitude') {
		// }
    		// if(targetField.name == 'Point Longitude') {
		// }
    		// if(targetField.name == 'Point Latitude') {
		// }
    		// if(targetField.name == 'Point Value') {
		// }
    		// if(targetField.name == '3D') {
		// }
    ...
    function checkMappingsAndParseData() {
		...
		// do various dimensionality checks here too, TODO;
		...
		somethingChanged = true;
    		...
    		for(var i = 0; i < dataMappings[src].map.length; i++) {
    		    if((dataMappings[src].vizType == 0 && (dataMappings[src].map[i].name == "Point Longitude"
							   || dataMappings[src].map[i].name == "Point Latitude"))
		       || (dataMappings[src].vizType == 1 && (dataMappings[src].map[i].name == "Start Longitude"
							      || dataMappings[src].map[i].name == "Start Latitude"
							      || dataMappings[src].map[i].name == "End Longitude"
							      || dataMappings[src].map[i].name == "End Latitude"))
		       || (dataMappings[src].vizType == 5 && (dataMappings[src].map[i].name == "Start Longitude"
							      || dataMappings[src].map[i].name == "Start Latitude"
							      || dataMappings[src].map[i].name == "End Longitude"
							      || dataMappings[src].map[i].name == "End Latitude"
							      || dataMappings[src].map[i].name == "Point Value"))
		       || (dataMappings[src].vizType == 2 && (dataMappings[src].map[i].name == "Point Longitude"
							      || dataMappings[src].map[i].name == "Point Latitude"
							      || dataMappings[src].map[i].name == "Point Value"))
		       || (dataMappings[src].vizType == 3 && (dataMappings[src].map[i].name == "Point Longitude"
							      || dataMappings[src].map[i].name == "Point Latitude"
							      || dataMappings[src].map[i].name == "3D"))
		       || (dataMappings[src].vizType == 4 && (dataMappings[src].map[i].name == "Point Longitude"
							      || dataMappings[src].map[i].name == "Point Latitude"
							      || dataMappings[src].map[i].name == "3D Cells"
							      || dataMappings[src].map[i].name == "3D"))
	...
	if($scope.doDebugLogging) {
	    $log.log("hopVizMapWebble: " + message);
	}
    }
	...
	for(var src = 0; src < dataMappings.length; src++) { 
		...
		if(dataMappings[src].vizType == 0 
		   || dataMappings[src].vizType == 1 || dataMappings[src].vizType == 5
		   || dataMappings[src].vizType == 2) {
		    ...
		    // if(haveLat && haveLon) {
		    // 	dataMappings[src].newSelectionsLat(myInstanceId, function(idx) { return mySelectionStatusLat(srcsrc, idx); }, true);
		    // 	dataMappings[src].newSelectionsLon(myInstanceId, function(idx) { return mySelectionStatusLon(srcsrc, idx); }, false);
		    // } else {
		    // 	if(haveLat) {
		    // 	    dataMappings[src].newSelectionsLat(myInstanceId, function(idx) { return mySelectionStatusLat(srcsrc, idx); }, false);
		    // 	}
		    // 	if(haveLon) {
		    // 	    dataMappings[src].newSelectionsLon(myInstanceId, function(idx) { return mySelectionStatusLon(srcsrc, idx); }, false);
		    // 	}
	...
	if(dataMappings[src].active) {
	    ...
	    } else if(dataMappings[src].vizType == 1
		      || dataMappings[src].vizType == 5) { // lines
		...
		if(lon1 === null
		   ...
		   || lat2 === null
		    ...
		    // --- This works well for large areas and short
		    // --- lines, not so well for very long lines and
		    // --- small areas (since the bounds of the line
		    // --- will include lots of things not very close
		    // --- to the line)
	...
	if(dataMappings[src].active) {
		...
		// var v = dataMappings[src].valFun(idx);
		// var groupId = 0;
	...
	var firstVal = true;
		...
		if(dataMappings[src].vizType == 0) {
			...
			var val = f(i);
			    ...
			    if(val.length < 1
			       || val[0].length != dataMappings[src].sizeLat
			       || val[0][0].length != dataMappings[src].sizeLon) {
				debugLog("Error: Sizes of 3D array and of longitude/latitude arrays do not match");
				dataIsCorrupt = true;
			    }
			} else {
			    NULLS++;
			...
			var lat = flat(i);
			if(lat === null) {
			    debugLog("Error: null values not allowed in longitude/latitude arrays");
			    dataIsCorrupt = true;
			} else {
			    if(firstLat) {
				firstLat = false;
		...
		if(dataMappings[src].vizType == 5) {
			...
			if(bounds.intersects(mapBounds)) {
				...
				if(lineMod == 0 || lineMod == 2) {
    				    // lw = 1 + lineWidth * (w - limits.minVal) / (limits.maxVal - limits.minVal);
				...
				if(drawPretty) {
					...
					// Jonas ToDo: now we redraw
					// with full alpha (ignoring
					// previously drawn unselected
					// data), maybe we should use
					// alpha. That makes
					// everything slower, but it
					// also makes places with lots
					// of unselected data very
					// very gray.
					//drawLineDDAfullalpha(pixels0, x1 + offset, y1, x2 + offset, y2, col[0], col[1], col[2], Math.ceil(col[3] * op), uCanvas.width, lw);
				...
				if(drawPretty) {
					...
					//drawLineDDAfullalpha(pixels0, x1 + offset, y1, x2 + offset, y2, col[0], col[1], col[2], Math.ceil(col[3] * op), uCanvas.width);
    ...
    function heatAndGroup2color(adjustedVal, groupId) {
	var v = Math.min(Math.max(0, adjustedVal), 1);
    	var rest = 1 - v;
	...
	var f1 = 1 - f;
    	...
    	var res = [Math.floor(rgba[0] * f + f1 * (col1[0] * rest + col2[0] * v)),
    		   Math.floor(rgba[1] * f + f1 * (col1[1] * rest + col2[1] * v)),
    		   Math.floor(rgba[2] * f + f1 * (col1[2] * rest + col2[2] * v)),
    		   rgba[3]];
    	return res;

	if(v > 0.5) {

    	    var res = [Math.floor(col1[0] * rest + col2[0] * v),
    		       Math.floor(col1[1] * rest + col2[1] * v),
    		       Math.floor(col1[2] * rest + col2[2] * v),
    		       rgba[3]];
    	    return res;
    ...
    function drawHeatmap(src) {
    	...
    	for(var span = 0; span < selections.length; span++) {
    		...
    		if(mapMaxLon < mapMinLon) {
    			...
    			// map crosses dateline, selection does not
    			// check which side of the dateline we are on
    		    ...
    		    if(selMaxLon < selMinLon) {
    			// only selection crosses dateline
    			// check which side of the dateline the map is on
    	...
    	if(atLeastOneSelectionOnScreen) { // if no selections are in the visible area, there will be nothing to draw
    		...
    		for(var i = 0; i < size; i++) {
    			...
    			if(mapWorldWraps) {
    			    ...
    			    var latInside = false;
    	    		    if((mapMinLat <= lat1 
    	    			&& mapMaxLat >= lat1) 
    			       || (Math.abs(lat1 - mapMinLat) <= limits.minLatDiff
    				   || Math.abs(lat1 - mapMaxLat) <= limits.minLatDiff)
    			      ) {
    				latInside = true;
    			    }
    				...
    				&& mapMinLon <= normalizedLon
    				   ...
    				   && (mapMinLon <= normalizedLon
    				       || mapMaxLon >= normalizedLon))
    			       || (Math.abs(lon1 - mapMinLon) <= limits.minLonDiff
    				   || Math.abs(lon1 - mapMaxLon) <= limits.minLonDiff)
    			      ) {
    				lonInside = true;
    			    }
    		...
    		for(var j = 0; j <= drawH; j++) {
    			...
    			if(voteMatrix[j][i].length > 0) {
    				...
    				allValues.push(av);
    			    } else if(colorMethod == 0 || colorMethod == 2 || colorMethod == 6 || colorMethod == 7
				      || colorMethod == 4 || colorMethod == 5 // the colorKey may also need min/max values
				     ) { // using min/max
    		...
    		for(var j = 0; j <= drawH; j++) {
    			...
    			if(voteMatrix[j][i].length > 0 && voteMatrix[j][i][0] !== null) { // why are there null here??
    			    // find how far to the left, right, top,
    			    // bottom we should go from here before
    			    // another cell with data is found

    			    // this breaks for diagonal stuff, we
    			    // assume neighboring cell is in same
    			    // column or row
    				...
    				if(voteMatrix[j][left].length > 0) {
    					...
    					if(voteMatrix[j][left][0] === null // why are there null here??
					   || voteMatrix[j][left][0][0] != myColor[0]
    					   || voteMatrix[j][left][0][1] != myColor[1]
    					   || voteMatrix[j][left][0][2] != myColor[2]) {
    					    sameColor = false;
    					}
    				    }
    				    break;
    				    ...
    				    // 	if(selectionsOnScreen[span][0] <= left
    				    // 	   && selectionsOnScreen[span][2] >= left) {
    				    // 	    inside = true;
    				    // 	    break;
    				    // 	}
    				    // }
    				...
    				if(voteMatrix[j][right].length > 0) {
    				    ...
    				    if(colorMethod == 5 || colorMethod == 7) { // curves
    					if(voteMatrix[j][right][0] === null
					   || voteMatrix[j][right][0][0] != myColor[0]
    					   || voteMatrix[j][right][0][1] != myColor[1]
    					   || voteMatrix[j][right][0][2] != myColor[2]) {
    					    sameColor = false;
    					}
    				    }
    				    break;
    				    ...
    				    // var inside = false;
    				    // for(var span = 0; span < selectionsOnScreen.length; span++) {
    				    // 	if(selectionsOnScreen[span][0] <= right
    				    // 	   && selectionsOnScreen[span][2] >= right) {
    				    // 	    inside = true;
    				    // 	    break;
    				    // 	}
    				    // }
    				...
    				if(voteMatrix[top][i].length > 0) {
    				    ...
    				    if(colorMethod == 5 || colorMethod == 7) { // curves
    					if(voteMatrix[top][i][0] === null
					   || voteMatrix[top][i][0][0] != myColor[0]
    					   || voteMatrix[top][i][0][1] != myColor[1]
    					   || voteMatrix[top][i][0][2] != myColor[2]) {
    					    sameColor = false;
    					}
    				    }
    				    break;
    				    ...
    				    // var inside = false;
    				    // for(var span = 0; span < selectionsOnScreen.length; span++) {
    				    // 	if(selectionsOnScreen[span][1] <= top
    				    // 	   && selectionsOnScreen[span][3] >= top) {
    				    // 	    inside = true;
    				    // 	    break;
    				    // 	}
    				    // }
    				...
    				if(voteMatrix[bot][i].length > 0) {
    				    ...
    				    if(colorMethod == 5 || colorMethod == 7) { // curves
    					if(voteMatrix[bot][i][0] === null
					   || voteMatrix[bot][i][0][0] != myColor[0]
    					   || voteMatrix[bot][i][0][1] != myColor[1]
    					   || voteMatrix[bot][i][0][2] != myColor[2]) {
    					    sameColor = false;
    					}
    				    }
    				    break;
    				    ...
    				    // var inside = false;
    				    // for(var span = 0; span < selectionsOnScreen.length; span++) {
    				    // 	if(selectionsOnScreen[span][1] <= bot
    				    // 	   && selectionsOnScreen[span][3] >= bot) {
    				    // 	    inside = true;
    				    // 	    break;
    				    // 	}
    				    // }
	...
	if(atLeastOneSelectionOnScreen) { // if no selections are in the visible area, there will be nothing to draw
		...
		for(var j = 0; j <= drawH; j++) {
			...
			for(var z = 0; z < dimZ; z++) {
			    // we currently do not listen to selections on Z, maybe do this in the future? TODO
				...
				if(fselLat(y) > 0) {
				    ...
				    var latInside = false;
    	    			    if((mapMinLat <= lat
    	    				&& mapMaxLat >= lat) 
				       || (Math.abs(lat - mapMinLat) <= limits.minLatDiff
					   || Math.abs(lat - mapMaxLat) <= limits.minLatDiff)
				      ) {
					latInside = true;
				    }
						...
						// if(v !== null) {
							...
							while(normalizedLon > 180) {
							   ...
							   || (passDateLine 
							       && (mapMinLon <= normalizedLon
								   || mapMaxLon >= normalizedLon))
							   || (Math.abs(lon - mapMinLon) <= limits.minLonDiff
							       || Math.abs(lon - mapMaxLon) <= limits.minLonDiff)
							  ) {
							    lonInside = true;
							}
							...
							if(x1 >= 0 && x1 <= drawW) {
							    ...
							    // we do not currently listen to the selections of individual cells, maybe do this in the future? TODO
    ...
    function drawData() {
    	for(var src = 0; src < dataMappings.length; src++) {
    	    if(dataMappings[src].active) {
		if(dataMappings[src].vizType == 0
		   || dataMappings[src].vizType == 1 || dataMappings[src].vizType == 5) { // points or lines
	...
	try {
	    ...
	    case "MapOpacity":
		// var newOp = Math.min(1, Math.max(0, eventData.slotValue));
