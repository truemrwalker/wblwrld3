...
wblwrld3App.controller('hopVizHaloMergeWebbleCtrl', function($scope, $log, Slot, Enum, $timeout) {
	...
	if($scope.doDebugLogging) {
	    $log.log("hopVizHaloMergeWebble: " + message);
	}
    }
    ...
    function mySelectionStatus(src, idx) {
	    ...
	    if(highlightedHalo.hasOwnProperty("src")
	       && highlightedHalo.src == src) {

		if(highlightedHalo.idx == idx) {
		    return selections.length + 1;
		}

		if(highlightedDescendants.hasOwnProperty("src")
		   && highlightedDescendants.src == src
		   && highlightedDescendants.hasOwnProperty("idxs")
		   && highlightedDescendants.idxs.hasOwnProperty(idx)) {
		    return selections.length + 2;
		}

		if(highlightedAncestors.hasOwnProperty("src")
		   && highlightedAncestors.src == src
		   && highlightedAncestors.hasOwnProperty("idxs")
		   && highlightedAncestors.idxs.hasOwnProperty(idx)) {
    		...
    		if(selections.length > 0) {
    			...
    			if(selections[span][0] <= px 
    			   && px <= selections[span][1]
    			   && selections[span][2] <= py 
    			   && py <= selections[span][3]) {
    			    groupId = span + 1;
    			    break;
    			}
    		    }
    ...
    function parseData() {
    	...
    	if(lenX != lenY
    	   || lenX != lenSize
    	   || (haveIDs && (lenID != lenChID
    			   || lenTime != lenID
    			   || lenTime != lenX))) {
    	    debugLog("Data fields have different numbers of data items.");
    	    dataIsCorrupt = true;
    	}
    			...
    			for(var j = i+1; j < dataMappings[src].size; j++) { // should appear later in the data. Note: there are halos with same IDs in other series, should we also check the "time" field from the original data to classify the series?
    				...
    				if(x1 === null
    				   ...
    				   || y2 === null
    				    ...
    				    if(!storyGraphMode
    				       && ((x1 < zoomMinX && x2 < zoomMinX)
    					   || (x1 > zoomMaxX && x2 > zoomMaxX)
    					   || (y1 < zoomMinY && y2 < zoomMinY)
    					   || (y1 > zoomMaxY && y2 > zoomMaxY)))
    				    {
	...
	if(!first) {
	    ...
	    if(!highlightedHalo.hasOwnProperty("idx")
