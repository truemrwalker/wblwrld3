...
wblwrld3App.controller('hopFileDataWebbleCtrl', function($scope, $log, Slot, Enum, $timeout) {
    ...
    var myInstanceId = -1; // supposedly unique identifier, used to
			   // keep track of different data sources

    var fullyLoaded = false; // flag to keep track if we are still
			     // being loaded (so we should avoid doing
			     // some things)
    ...
    // --------------------------------------------------------------------------------
    // Things for keeping track of selected subsets of the data and
    // updating all listening visualization components about changes
    // in selection status.
    // --------------------------------------------------------------------------------

    var selectionUpdateNumber = 0; // Components may be listening to
				   // more than one field, and will
				   // then get multiple messages that
				   // the selection status has
				   // changed. This number helps them
				   // keep track of whether to redraw
				   // again or not.

    var dataUpdateNumber = 0; // Same, but for updates when the data
			      // contents change.

    var sendingDataMess = false; // flag to keep track of if we hare
				 // sending "newData" messages (which
				 // will generally generate lots of
				 // "newSelections" messages from the
				 // visualization components) or not.


    $scope.pauseUpdates = function(pauseNow) {
	// used by other Webbles that know
	// more than we do to tell use to
	// pause updates for a while.
    ...
    // flags to track what parts of the data have new selections, to
    // know which listeners to update
    ...
    var dataListeners = []; // one list item for each feature. each item contains the info for the interested visualization component
	...
	sendingDataMess = true; // ignore all "newSelections" messages
				// while we tell everyone about the
				// new data
	dataUpdateNumber++;
	...
	sendNewSelectionMessageAll(); // tell everyone that the
				      // selections are also changed
    }

    // Tell all listening components that we have loaded new data that
    // is not compatible with what they had before, so they should
    // reset
    function sendGiveUpMessage() {
	debugLog("Data format has changed to something incompatible, tell everyone to clear out their visualization settings.");
	sendingDataMess = true; // ignore all "newSelections" messages
				// while we tell everyone about the
				// new data
	dataUpdateNumber++;
    ...
    // Selection status of the data items 
    // This sends the actual update messages to visualization
    // components listening to updates on various data fields.
	...
	// TODO: we need to listen for updates on this Webble getting
	// deleted, so we can clean up our list when that happens.

	if(!mapVizToNumber.hasOwnProperty(newViz)) {
	    var no = mapVizToNumber.next;
	    mapVizToNumber.next = mapVizToNumber.next + 1;
	    
	    mapVizToNumber[newViz] = Math.pow(8, no); // TODO: this part
						   // should probably
						   // be done
						   // better. Or at
						   // least be
	...
	if(featureIdx < dataFields.length) {
		...
		if(vizSelectionFunctions[featureIdx][viz][1] == newViz) {
		    // this visualization component already registered
		    // a function and wants to replace it
		    add = false;
		    
		    if(vizSelectionFunctions[featureIdx][viz][0] === null
		       && fun === null) {
	    ...
	    if(fun !== null) {
		debugLog("Request for registering new selections on an unknown feature.");
	    }
	    dirty = false;
	}
	...
	for(var featureIdx = 0; featureIdx < dataFields.length; featureIdx++) {
		...
		if(dataFields[featureIdx].masters[feat][1] === null) { // no remap of value index, assume we can use the same cache
	...
	if(featureIdx < dataFields.length) {
	    ...
	    if(valueIdx < vizSelectionsCache[fRep].length
	       && vizSelectionsCache[fRep][valueIdx] >= 0) { // cache hit
		...
		for(var viz = 0; viz < vizSelectionFunctions[f].length; viz++) {
		    ...
		    if(vizSelectionFunctions[f][viz][0] === null) { // this component is no longer visualizing this data
			// nothing to do
		    } else {
			...
			if(!found) { 
			    if(vizSelectionFunctions[f][viz][2]) { // if we have a function that selects everything
				groupId = 1;
			    } else {
				var vIdx = valueIdx;
    ...
    // Update this to change the visual representation
    // (angular will automatically update according to this list)
    ...
    //===================================================================================
    // We have density data from the Tokyo University group that is
    // binary data and always has a fixed size. Check the file size of
    // binary files to see if they may contain this type of data.
    ...
    //===================================================================================
    // Rockstar is a binary data format used by several astrophysics
    // systems. Rockstar files always start with a 64 bit magic
    // number. Check if a file starts with this number and if so
    // assume it to be Rockstar format data.
	...
	debugLog("inputURL changed: '" + $scope.inputURL + "'");
	if($scope.inputURL !== undefined 
	   && $scope.inputURL != "") {
	    debugLog("inputURL is valid URL: '" + $scope.inputURL + "'");


	    resetVars(); // we may get several files, so clear out old
	    // data now and then do not clear when
	    // reading files
		...
		// assume text format, some form of comma or tab separated stuff (or JSON)
		...
		// rockstar format has magic number
		// density format has fixed file size
		// if we have ctl file, guess GrADS
    		haveDats++;
		...
		// some binary format, read files and check for
		// different types based on file contents
    ...
    $scope.onFilesAdded = function(files) {
	    ...
	    // If so, send files off to a function that can handle
	    // that format.
		...
		if(fn.indexOf(".gz") >= 0) {  // add all other supported file types too?
		    ...
		    // Start reading netCDF files immediately, but do
		    // this only once
    ...
    // trim whitespace from start and end of strings and replace multiple whitespace with one space
    function trim(str) {
    ...
    // this is used to do debug logging during development, and setting 
    // the flag to false removes all debug messages that are still left
    $scope.doDebugLogging = true;
    function debugLog(message) {
    	if($scope.doDebugLogging) {
    	    $log.log("hopFileDataWebble: " + message);
    	}
    }

    // counts the number of times a certain character occurrs in a string
	...
	if(ver > 0) {
		...
		if(temp != 0) {
		    debugLog("expected another 0 when dimension array is absent");
		...
		for(var d = 0; !dataFileIsCorrupt && d < dimArrN; d++) {
		    ...
		    if(l == 0) {
			// debugLog("Found record dimension? dim " + d + " has dim length " + l);
			if(recordDimension >= 0) {
			    debugLog("ERROR: Found more than one record dimension!!");
			}
			recordDimension = d;
		    }
		...
		debugLog("ERROR: unknown tag when expecting 0 or dimension array.");
		dataFileIsCorrupt = true;
	    }
		...
		if(temp != 0) {
		    debugLog("expected another 0 when global attribute array is absent");
		    dataFileIsCorrupt = true;
		}
		...
		for(var d = 0; !dataFileIsCorrupt && d < gattArrN; d++) {
			...
			debugLog("ERROR: unknown data type in global attributes array.");
			dataFileIsCorrupt = true;
		    }
		...
		if(temp != 0) {
		    debugLog("expected another 0 when variable array is absent");
		    dataFileIsCorrupt = true;
		}
		...
		for(var varArrD = 0; !dataFileIsCorrupt && varArrD < varArrN; varArrD++) {
			...
			if(temp != 0) {
			    debugLog("expected another 0 when variable attribute array is absent");
			    dataFileIsCorrupt = true;
			}
			...
			debugLog("ERROR: unknown data type in variable array.");
			dataFileIsCorrupt = true;			
		    }
			...
			var temp = dv.getUint32(offset, false);
			offset += 4;
			debugLog("ERROR: we cannot deal with 64-bit offsets currently.");
			dataFileIsCorrupt = true;
		    }
		...
		if(dimIdLs.length > 0 && dimIdLs[0] == recordDimension) {
			...
			case 6: // double
			    ...
			    // debugLog("reading one 64 bit float: " + temp1.toString(16) + " " + temp2.toString(16) + " and got " + val + " at offset " + myOffset + " (" + (myOffset % 4) + " " + (myOffset % 8) + ")");
			    var dddd = new Date(1858,11,17);
			    var dddd2 = new Date(dddd.getTime() + 24*60*60*1000 * val);
			    // debugLog("if 64 bit float is days since 1858-11-17, this represents " + dddd2);
			...
			if(dimIdLs.length == 1) {
			    ...
			    debugLog("WARNING: dimensionality is higher than this system expects. Making one big 1-dimensional vector.");
	    ...
	    // debugLog("found " + records.length + " records in the file");
	    debugLog("found " + noofRecords + " records in the file");
	}



	if(dataFileIsCorrupt) {
	    debugLog("ERROR: File is not a correctly formatted netCDF file, " + myFileName);
	} 
    ...
    // var netCDFnames = ["", "BYTE", "CHAR", "SHORT", "INT", "FLOAT", "DOUBLE"];
    // var netCDFsizes = [0, 1, 1, 2, 4, 4, 8];

    function checkIfNetCDF(dv) {
	// debugLog("Check if file is netCDF");
	...
	for(var f = 0; f < data.fieldNames.length; f++) {
	    ...
	    fieldInf.slaves = [f]; // fields that need updating when this field changes
	    fieldInf.masters = [[f, null]]; // fields that when change
					    // also affects this
	...
	var masters = []; // we would like to add the longitude and latitude fields (now vectors) TODO!
	...
	for(var f = ourStartIdx; f < data.fieldNames.length; f++) {
		...
		if(first) {
		    ...
		    if(bandFieldIdx >= 0) { // we have "band" data
			var fieldInf = {};
		    ...
		    // if(timeFieldIdx >= 0) { // we have "timestamp" data
		    // 	var fieldInf = {};
	...
	try {
	    ...
	    if(jsonContents.hasOwnProperty("fieldNames")
	       && jsonContents.hasOwnProperty("fieldTypes")
	       && jsonContents.hasOwnProperty("columns")) {
		data = {};
		data.fieldNames = [];
		data.fieldTypes = [];
		data.columns = [];
		...
		for(var f = 0; f < jsonContents.columns.length; f++) {
    		...
    		fieldInf.name = data.fieldNames[f];
    		fieldInf.type = [data.fieldTypes[f]];
		if(fieldInf.type[0] == "latitude"
		   || fieldInf.type[0] == "longitude") {
	...
	if(checkIfRockstar(contents)) {
	    ...
	    // Guess this is flooding data from the Tohoku University group
	    // should be 330400 bytes, 350x236 32bit floats

	    // debugLog("This looks like tsunami damage data");	    

	    parseTohokuBuildingDamageData(contents, reader.lastFile);
	    
	} else if(reader.fileName.indexOf("inund") == 0) {
	    // Guess this is flooding data from the Tohoku University group
	    // should be 33133824 bytes, 3504x2364 32bit floats

	    // debugLog("This looks like tsunami flooding data");
	    ...
	    debugLog("This is either an unknown binary format or text data. Retrying as text data.");
	...
	if(tmpFloat.length != 82600) { 
	    debugLog("Error: Unexpected file size of tsunami damage data.");
	    return;
	}
	...
	// filenames can be "inund[0-9]+.dat" for max inundation or
	// "inund5000xxx.dat", where xxx goes from 000 to 360 and is
	// the 30-second interval after the earthquake (total 3hrs)

	var littleEndian = true;
	var tmpFloat = new Float32Array(contents);

	if(tmpFloat.length != 8283456) { 
	    debugLog("Error: Unexpected file size of flooding data.");
	    return;
	}
	...
	// for(var j = 0; j < 2364; j++) {
	//     for(var i = 0; i < 3504; i++) {
	// 	data[j][i] = dv.getFloat32(offset, littleEndian);
	// 	offset += 4;
	//     }
	// }
	...
	if(p > 3) {
	    ...
	    // debugLog("get30secondIntervalFromFileName(" + fileName + ") -> " + p + " " + s + " -> " + res);
    ...
    // function convertTohokuData(flooding, timeSlices, data2ds) {
	...
	// first add all flooding data
	// then add all non-flooding data
	...
	for(var pass = 0; pass < 2; pass++) {
	    ...
	    // also affects this field,
	    // and a function to transform
	    // the index to the
	    // appropriate value
    ...
    //===================================================================================
    //======================== Parsing the Rockstar file format =========================
    //===================================================================================
    ...
    //===================================================================================
    //======================== Parsing the "density" file format ========================
    //===================================================================================

    // This format comes from the Tokyo University astrophysics
    // group. a 256x256x256 binary grid of density data.
	...
	// data = {}; // this should not be reset here, TODO (what if we have files of different types?)
	// data.fieldNames = ["Density 3D Array", "File No."];
	// data.fieldTypes = [["3Darray"], ["number"]];
	// data.columns = [[], []];
    ...
    // converts a string with a date in GrADS format to a date object
    function parseGrADSdate(str) {
    	...
    	// this should strictly speaking not happen according to the standard, 
    	// but in our example data files there was no space 
    	// between the date and the time increment specification
    	var extra = "";
    	if(str.length > c + 7) {
    ...
    // parse all the binary data files in the GrADS format
    ...
    // parse one binary data file in the GrADS format
    function gradsBinFileReaderOnLoadCallback(e2, reader, header, fname, startFidx) {

    	// debugLog("gradsBinFileReaderOnLoadCallback");
    		...
    		for(var v = 0; v < header.vars.length; v++) {
    			...
    			for(var y = 0; y < header.ydef.length; y++) {
    				...
    				if(val != header.undefVal) {
    				    ...
    				    // if(val > 0) {
    				    // 	debugLog("val: " + val + " z:" + z + ", y:" + y + ", x:" + cube[z][y].length);
    				    // }
    ...
    // the ensemble value can be encoded in the filename, if it is, this is how to read it back
    ...
    // the time stamp value can be encoded in the filename, and if it is use this to get the value
    ...
    // var listOf3DfeatureToDependent1Dfeatures = {};
    // var listOf3DfeatureToDependent1DfeaturesLs = [];
	...
	// listOf3DfeatureToDependent1Dfeatures = {};
	// listOf3DfeatureToDependent1DfeaturesLs = [];
	...
	// fieldInf.sel = xSelected;
    	// fieldInf.newSel = addXSelection;
	...
	fieldInf.listen = addListeningViz;
 	...
 	// fieldInf.sel = ySelected;
    	// fieldInf.newSel = addYSelection;
	...
	// fieldInf.sel = zSelected;
    	// fieldInf.newSel = addZSelection;
	...
	// this is handled by "parsingFinished()"
    	// updateView();
	...
	return function(idx) {
	    // debugLog("function for T or E, idx " + col) ;  
    ...
    // react to changes on our input slots
    ...
    // returns one complete 3D cube
    function get3DdataAs3D(feature, idx) {
	if(feature >= 0 
	   && idx >= 0 
	   && feature < dataFields.length 
	   && feature < data.columns.length 
	   && idx < data.columns[feature].length
	   && dataFields[feature].type[0] == "3Darray") {
    ...
    function transformCellIdxToCubeIdx(idx, featureIdx) { // we need the featureIdx because different data fields can have different number of height levels (z.length)
	if(featureIdx >= 0 
	   && idx >= 0 
	   && featureIdx < dataFields.length 
	   && featureIdx < data.columns.length 
	   && dataFields[featureIdx].type[0] == "3Darray") {

	    var cubeNo = 0;

	    if(data.columns[featureIdx].length > 0
	       && data.columns[featureIdx][0].length > 0
	       && data.columns[featureIdx][0][0].length > 0
	       && data.columns[featureIdx][0][0][0].length > 0
	      ) {
	...
	if(feature >= 0 
	   ...
	   && dataFields[feature].type[0] == "3Darray") {
	    ...
	    if(data.columns[feature].length > 0
	       && data.columns[feature][0].length > 0
	       && data.columns[feature][0][0].length > 0
	       && data.columns[feature][0][0][0].length > 0
	      ) {
    ...
    // returns the value in one cell in a 3D cube
	...
	// $scope.addSlot(new Slot('OverrideNullvalue',
    	// 			0,
    	// 			"Override Null Value",
    	// 			'Force interpretation of some value to be null, or trust the file specification (which may or may not include such a value).',
    	// 			$scope.theWblMetadata['templateid'],
    	// 			{inputType: Enum.aopInputTypes.ComboBoxUseIndex, comboBoxContent: ["Trust Specification", "Force Little Endian", "Force Big Endian"]},
    	// 			undefined
    	// 		       ));
	...
	// $scope.addSlot(new Slot('SelectionsChanged',
    	// 			false,
    	// 			'Selections Changed',
    	// 			'This slot changes value (between true and false) when the selection status of data provided even if the format and the data did not change.', 
    	// 			$scope.theWblMetadata['templateid'],
    	// 			undefined,                                 
    	// 			undefined
    	// 		       ));
