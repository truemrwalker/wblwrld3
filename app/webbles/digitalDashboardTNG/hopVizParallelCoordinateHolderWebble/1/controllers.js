//======================================================================================================================
// Controllers for Digital Dashboard 3.0 TNG HoP Parallel Coordinate Holder Visualisation Webble for Webble World v3.0 (2013)
...
wblwrld3App.controller('hopVizParallelCoordinateHolderWebbleCtrl', function($scope, $log, $timeout, Slot, Enum) {
	...
	$scope.dataSetName = "";
	var preDebugMsg = "hopVizParallelCoordinateWebble: ";
		...
		for(var src = 0; src < dataMappings.length; src++) {
			...
			if(canActivate) {
				...
				for(var i = 0; i < dataMappings[src].map.length; i++) {
					...
					if(dataMappings[src].map[i].active && dataMappings[src].map[i].hasOwnProperty("listen") && dataMappings[src].map[i].liste !== null) {
		...
		for(coord = 0; coord < coordTypes.length; coord++) {
			...
			for(sel = 0; sel < selections[coord].length; sel++) {
				if(selections[coord][4]) {
					...
					result.selections[coord].push({'min':selections[coord][2], 'max':selections[coord][3], 'onlyNull':selections[coord][4], 'includesNull':selections[coord][5], 'type':selections[coord][7]});
				}
			}
		}
				...
				if(unique > 0) {
					for(var sel = 0; sel < slotSelections.selections[coord].length; sel++) {
						...
						if(newSel.type != coordTypes[coord]) {
							...
							newSelections.push([topMarg + drawH + nullMarg / 2, topMarg + drawH + nullMarg, null, null, true, true, coord, coordTypes[coord]]);
						}
	...
	function checkSelectionsAfterNewData() {
		...
		for(coord = 0; coord < coordTypes.length; coord++) {
			...
			if(selections[coord].length <= 0 || coordTypes[coord] != selections[coord][0][7]) {
				selectAllOneCoord(coord);
			}
			else {
				var newSelections = [];
				...
				for(sel = 0; sel < selections[coord].length; sel++) {
					...
					if(selections[coord][sel][4]) { // only null
						newSel = [topMarg + drawH + nullMarg / 2, topMarg + drawH + nullMarg, null, null, true, true, coord, coordTypes[coord]];
					}
					else {
						if(coordTypes[coord] == 'string') {
				...
				if(dataMappings[src].active) {
					...
					if(dataMappings[src].hasOwnProperty("newSelections") && dataMappings[src].newSelections !== null) {
						if(!selectAllFlag) {
							...
							for(coord = 0; coord < coordTypes.length; coord++) {
								if(coordTypes[coord] == 'string') {
									if(selections[coord].length != 1 || selections[coord][0][2] > 0 || selections[coord][0][3] < coordLimits[coord].noofLabels || !selections[coord][0][5]) {
										selectAllFlag = false;
										break;
									}
								}
								else if(coordTypes[coord] == 'date') {
									if(selections[coord].length != 1 || selections[coord][0][2] > coordLimits[coord].min || selections[coord][0][3] < coordLimits[coord].max || !selections[coord][0][5]) {
										selectAllFlag = false;
										break;
									}
								}
	...
	function parseData() {
		...
		if(!alreadyDrewSomething) {
			// $log.log(preDebugMsg + "No data to draw.");
			resetVars();
		}
		else {
			var xpos = leftMarg;
	...
	function newSelection(Y1,Y2, coord, keepOld) {
		...
		if(coord >= 0 && coord < coordXpos.length) {
			...
			var newSel = [];
			if(onlyNull) {
				newSel = [topMarg + drawH + nullMarg / 2, topMarg + drawH + nullMarg, null, null, onlyNull, includesNull, coord, coordTypes[coord], coordTypes[coord]];
			}
			else {
				if(coordTypes[coord] == 'string') {
					...
					if(somethingInside) {
						...
						if(includesNull) {
							newSel = [topMarg + drawH + nullMarg / 2, topMarg + drawH + nullMarg, null, null, true, includesNull, coord, coordTypes[coord]];
						}
						else {
							newSel = [];
						}
	...
	//===================================================================================
	// Fake Drop No Redraw
	// This method does a fake data drop without calling for graphic redraw.
	//===================================================================================
	...
	// This method converts a pixel to a display value.
	//===================================================================================
	...
	// This method converts a pixel to a select value.
	//===================================================================================
	...
	//===================================================================================
	// Includes
	// This method checks whether an item is included in a specific element.
	//===================================================================================
	...
	// This method returns the nearest header from a specific pixel.
	//===================================================================================
	...
	// This method returns the nearest coordinate from a specific pixel.
	//===================================================================================
	...
	// This method draws the coordinates bars.
	//===================================================================================
	...
	// This method draws the coordinates bar's shadows.
	//===================================================================================
	...
	// This method draws the coordinates labels.
	//===================================================================================
	...
	//===================================================================================
	// Draw Poly Lines
	// This method draws the poly lines connecting the parallel coordinates.
	//===================================================================================
				...
				for(y2 = 0; y2 < votes[y1].length; y2++) {
					if(votes[y1][y2].length > 0) {
						...
						if(ls.length >= 1 && ls[0] > 0) { // draw unselected data line
							...
							if(transparency >= 1) {
								var color = hexColorToRGBAvec(legacyDDSupLib.getColorForGroup(groupId, colorPalette, currentColors), 0.33);
							}
							else {
	...
	//===================================================================================
	// Update Selecton Display Sizes
	// This method updates the selection display sizes.
	//===================================================================================
	...
	// This method selects all data points for a specific coordinate.
	//===================================================================================
	...
	// This method checks if the mouse is positioned within the header area.
	//===================================================================================
	...
	//===================================================================================
	// Mouse Position is Just Above Header Area
	// This method checks if the mouse is positioned just above the header area.
	//===================================================================================
	...
	//===================================================================================
	// Move Coordinates
	// This method manages to move the coordinates within the holder.
	//===================================================================================
	...
	//===================================================================================
	// Mouse Up or Out
	// This method checks if the mouse buton is up or the mouse pointer is outside
	// clickable area.
	//===================================================================================
