...
wblwrld3App.controller('digitalDashboardWebbleCtrl', function($scope, $log, Slot, Enum, jsonQuery) {
    ...
    function resetVars() {
	// if the same workspace is loaded twice, these are not reset properly, so we have to do it ourselves...
	...
	// debugLog("closeConnectionform");
		...
		for(var set = 0; set < listOfPlugins[p].format.length; set++) {
			...
			if(oldFields[f1].added) { // only added fields may have been removed
				...
				if(newFields[f2].added
				   && newFields[f2].assigned === oldFields[f1].assigned) { // this works?
				    stillHere = true;
				    break;
				}
			    }
			...
			if(newFields[f1].added) { // only added fields may have been added
				...
				if(oldFields[f2].added 
				   && oldFields[f2].assigned === newFields[f1].assigned) { // this works?
				    isNew = false;
				    break;
				}
			    }
				...
				for(var f2 = 0; f2 < oldFields.length; f2++) {
				    if(oldFields[f2].template
				       && oldFields[f2].name == newField.name) {
	...
	if(!webble) {
		...
		if(! (idx < listOfPlugins.length)) {
		    ...
		    if(!(slotContent 
			 && slotContent.hasOwnProperty("dataSourceField")
			 && slotContent.hasOwnProperty("pluginField"))) {
			...
			if(datasrc && datasrc.hasOwnProperty("vizName")) {
				...
				if(listOfPlugins[idx2].name == datasrc.vizName) {
					...
					if(listOfPlugins[idx2].format[set].filled) { // first filled set, this is the active one
						...
						if(listOfPlugins[idx2].format[set].fields[ff2].name == datasrc.name
						   && ((!listOfPlugins[idx2].format[set].fields[ff2].added
							&& !listOfPlugins[idx2].format[set].fields[ff2].template)
						       || (listOfPlugins[idx2].format[set].fields[ff2].added
							   && datasrc.idx == ff2))) {
			...
			if(!viz2viz &&
			   ! (datasrc 
			      && datasrc.hasOwnProperty("sourceName") 
			      && mapDataSourceNameToId.hasOwnProperty(datasrc.sourceName))) {
				...
				if(! (dsIdx < listOfDataSources.length
				      && datasrc.hasOwnProperty("dataSetIdx")
				      && datasrc.dataSetIdx < listOfDataSources[dsIdx].dataSets.length
				      && datasrc.hasOwnProperty("fieldName"))) {
			...
			for(var a = 0; a < listToAssign.length; a++) {
				...
				for(var f = 0; f < fields.length; f++) {
					...
					if(fields[f].added && slotContent.pluginField.added 
					   && f == slotContent.pluginField.idx) {
					    // this is the cloned field we want to update
					    match = true;
					}
					if(!fields[f].added 
					   && !fields[f].template
					   && !slotContent.pluginField.added
					   && !slotContent.pluginField.template) {
					    // normal field
					    match = true;
					}
					...
					if(match) {
						...
						if(fields[f].type[tt] == listOfDataSources[dsIdx].dataSets[dataSetIdx].fields[ff].type) {								    // acceptable data type
							...
							if(fields[f].hasOwnProperty("assigned")
							   && removePreviousStuff) {
							    fields[f].assigned = {};
							}
							if(!fields[f].hasOwnProperty("assigned")
							   || !fields[f].assigned.hasOwnProperty(assignedID)
							   || fields[f].assigned[assignedID].toString() != newVal.toString()) {
	...
	for(p = 0; p < needsData.length; p++) {
	    ...
	    // var webble = $scope.getWebbleByInstanceId(listOfPlugins[needsData[p]].id);
	    // webble.scope().forceReparseData(); // should not be necessary
	}
	...
	for(var p = 0; p < listOfPlugins.length; p++) {
		...
		for(var f = 0; f < inputSet.length; f++) {
			...
			for(var a in inputField.assigned) {
				...
				if(mapDataSourceIdToIdx.hasOwnProperty(dsID)) {
				    ...
				    if(ds < listOfDataSources.length 
				       && inputField.assigned[a][1] < listOfDataSources[ds].dataSets.length
				       && inputField.assigned[a][2] < listOfDataSources[ds].dataSets[inputField.assigned[a][1]].fields.length) {
		...
		if(!plugin.active // if it is already activated, a set that did not change is already used so other changes are not important
		   && (plugin.format[is].filled != setIsFilled
		       || JSON.stringify(plugin.format[is].sources) != JSON.stringify(OKsourcesPerIdSlot)
		      )) {
		    changed = true;
		}
	...
	if(somethingWrong || !newMapping.hasOwnProperty("plugins")) {
		...
		for(var set = 0; !somethingWrong && set < mplugin.sets.length; set++) {
			...
			for(var a = 0; !somethingWrong && a < mfields[f].assigned.length; a++) {				
			    ...
			    var dsIdx = mapDataSourceIdToIdx[dsId];
			    if(dsIdx >= listOfDataSources.length
			       || mfields[f].assigned[a].dataSetIdx >= listOfDataSources[dsIdx].dataSets.length) {
	...
	if(somethingWrong) {
		...
		for(var set = 0; set < mplugin.sets.length; set++) {
			...
			for(var a = 0; a < mfields[f].assigned.length; a++) {				
			    ...
			    if(!oldAssigned.hasOwnProperty(assignedID) 
			       || oldAssigned[assignedID][2] != newVal[2])
			    {
				dirty[p] = true;
			    }
			}
		...
		for(var idSlot in sourcesPerIdSlot) {
			...
			if(inputField.template && !inputField.added) {
				...
				for(var aa = 0; aa < sources.length; aa++) {
					...
					if(idSlotsMapped.hasOwnProperty(idSlotCheckPlugin)
	...
	if(Object.keys(colorDictionary).length <= 1
	...
	$scope.addSlot(new Slot('Colors',                  // Name
				...
				'Colors',                                  // Display Name
				'Colors to use for different groups of data',             // Description
				$scope.theWblMetadata['templateid'],        // Category (common to set to the template id)
				undefined,                                 
				undefined
			       ));
	
	$scope.addSlot(new Slot('Mapping',                  // Name
				{},                              // Value
				'Mapping',                                  // Display Name
				'Mapping of data fields and visualization input fields, i.e. what data should be visualized where?',             // Description
				$scope.theWblMetadata['templateid'],        // Category (common to set to the template id)
				undefined,                                 
				undefined
			       ));
