//======================================================================================================================
// Controllers for Digital Dashboard Plugin Map Flow Webble for Webble World v3.0 (2013)
...
//=======================================================================================
wblwrld3App.controller('mapFlowPluginWebbleCtrl', function($scope, $log, $timeout, Slot, Enum, dbService) {
	...
	var preDebugMsg = "DigitalDashboard Trajectories Map Flow: ";
	...
	function parseData() {
		...
		if(atLeastOneFilled) {
			...
			if(idArrays.length != lons1.length || idArrays.length != lats1.length || idArrays.length != timestamps.length || idArrays.length != ids.length) {
				dataIsCorrupt = true;
			}
			...
			if(!dataIsCorrupt) {
				...
				for(var source = 0; source < sources; source++) {
					...
					if(idArray.length != lons1[source].length || idArray.length != lats1[source].length || idArray.length != timestamps[source].length || idArray.length != ids[source].length) {
						dataIsCorrupt = true;
					}
					if(idArray.length <= 0) {
						dataIsCorrupt = true;
			...
			if(!dataIsCorrupt) {
				...
				for(var source = 0; source < lats1.length; source++) {
					...
					for(i = 0; i < Ns[source]; i++) {
						...
						if(lon1[i] !== null && lat1[i] !== null && timestamps[source][i] !== null && ids[source][i] !== null) {
							unique++;
							var x1 = lat1[i];
							var y1 = lon1[i];
	...
	function drawFlow() {
		...
		for(var pass = 0; pass < 2; pass++) {
			for(var trIdx = 0; trIdx < trajectories.length; trIdx++) {
				...
				if(!selected && pass == 0 || selected && pass > 0) {
						...
						for(var i = 0; i < ls.length; i++) {
							...
							if(drawPretty) {
								if(groupId <= 0) {
									...
									col = legacyDDSupLib.getColorForGroup(groupId, colorPalette, currentColors)
									if(transparency >= 1) {
