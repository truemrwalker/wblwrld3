//======================================================================================================================
// Controllers for Digital Dashboard Plugin Halo Merger Plot Webble for Webble World v3.0 (2013)
...
wblwrld3App.controller('haloMergerPlotPluginWebbleCtrl', function($scope, $log, Slot, Enum) {
	...
	var preDebugMsg = "DigitalDashboard Halo Merger Plot: ";
	...
	function parseData() {
			...
			for(var i = 0; i < parentInput.length; i++) {
				...
				if(parentInput[i].hasOwnProperty("name") && parentInput[i].name == "Size") {
					...
					if(parentInput[i].hasOwnProperty("description")) {
						...
						sName =  legacyDDSupLib.shortenName(parentInput[i]["description"]);
		...
		if(atLeastOneFilled) {
			...
			if(idArrays.length != xArrays.length || idArrays.length != yArrays.length || idArrays.length != sArrays.length) {
				dataIsCorrupt = true;
			}
			if(haveIDs) {
				...
				if(idArrays.length != hArrays.length || idArrays.length != dArrays.length || idArrays.length != tArrays.length) {
					dataIsCorrupt = true;
				}
			}
			...
			if(!dataIsCorrupt) {
				...
				for(var source = 0; source < sources; source++) {
					...
					if(idArray.length != xArray.length || idArray.length != yArray.length || idArray.length != sArray.length) {
						dataIsCorrupt = true;
					}
					if(haveIDs) {
						...
						if(idArray.length != hArray.length || idArray.length != dArray.length || idArray.length != tArray.length) {
							dataIsCorrupt = true;
						}
					}
			...
			if(!dataIsCorrupt) {
				...
				for(var source = 0; source < sources; source++) {
					...
					for(i = 0; i < Ns[source]; i++) {
						...
						if(xArray[i] !== null && yArray[i] !== null && sArray[i] !== null) {
							unique++;
							var x = xArray[i];
							var y = yArray[i];
							var s = sArray[i];
	...
	//===================================================================================
	// Draw Lines
	// This method draws all the lines.
	//===================================================================================
					...
					for(var j = i+1; j < Ns[set]; j++) { // should appear later in the data. Note: there are halos with same IDs in other series, should we also check the "time" field from the original data to classify the series?
						if(tArray[j-1] > tArray[j]) {
							...
							if(xArray[i] === nul || yArray[i] === null || sArray[i] === null || xArray[j] === null || yArray[j] === null || sArray[j] === null) {
								...
								if(!storyGraphMode && ((xArray[i] < zoomMinX && xArray[j] < zoomMinX) || (xArray[i] > zoomMaxX && xArray[j] > zoomMaxX) || (yArray[i] < zoomMinY && yArray[j] < zoomMinY) || (yArray[i] > zoomMaxY && yArray[j] > zoomMaxY))){
	...
	//===================================================================================
	// Draw Halos
	// This method draws all the Halos.
	//===================================================================================
	function drawHalos(W, H) {
		...
		for(var set = 0; set < Ns.length; set++) {
			...
			for(var i = 0; i < Ns[set]; i++) {
				if(xArray[i] === null || yArray[i] === null || sArray[i] === null || (storyGraphMode && tArray[i] === null)) {
					continue;
				}

				if(!storyGraphMode && (xArray[i] < zoomMinX || xArray[i] > zoomMaxX || yArray[i] < zoomMinY || yArray[i] > zoomMaxY)) {
		...
		for(var set = 0; set < Ns.length; set++) {
			...
			for(var i = 0; i < Ns[set]; i++) {
				...
				if(!storyGraphMode &&
					(xArray[i] < zoomMinX || xArray[i] > zoomMaxX || yArray[i] < zoomMinY || yArray[i] > zoomMaxY)) {
					continue; // outside zoomed range
				}
	...
	// This method updates selections when zooming or resizing.
	//===================================================================================
	...
	// This method checks if globals selections actually changed..
	//===================================================================================
	...
	//===================================================================================
	// Select Closest Halo
	// This method selects the closes halo to the mouse pointer.
	//===================================================================================
