		...
		if(dArray[i] >= 0) { // this halo has a descendant, so we should draw a line
			...
			if(tArray[j-1] > tArray[j]) {
				       ...
				       || (yArray[i] > zoomMaxY && yArray[j] > zoomMaxY)))
				{
					...
					var x1 = val2pixelX(xArray[i]);
					var y1 = val2pixelY(yArray[i]);
					var x2 = val2pixelX(xArray[j]);
					var y2 = val2pixelY(yArray[j]);
				    }
	...
	for(var set = 0; set < idArrays.length; set++) {
		...
		if(xArray[i] !== null
		   && yArray[i] !== null) {
			...
			var dx = val2pixelX(xArray[i]) - currentMouse.x;
			var dy = val2pixelY(yArray[i]) - currentMouse.y;
		    }
