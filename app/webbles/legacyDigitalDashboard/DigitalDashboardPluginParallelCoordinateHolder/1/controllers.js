//======================================================================================================================
// Controllers for Digital Dashboard Plugin Parallel Coordinate Holder for Webble World v3.0 (2013)
...
wblwrld3App.controller('parallelCoordinateHolderPluginWebbleCtrl', function($scope, $log, Slot, Enum) {
	...
	$scope.dataSetName = "";
	var preDebugMsg = "DigitalDashboard Parallel Coordinates Holder: ";
	...
	function localHexColorToRGBAvec(color, alpha) {
		var res = [];
	...
	//===================================================================================
	// Local Hex Color to RGBA Vector
	// This method converts a hex color to a RGBA Vector.
	// Since this Webbles seem to manage alpha a bit different then other plugins (?) it
	// will use a local version of this function instead of the legacy Library version.
	//===================================================================================
	function localHexColorToRGBAvec(color, alpha) {
		var res = [];
			...
			for(coord = 0; coord < coordTypes.length && coord < slotSelections.selections.length; coord++) {
				...
				if(unique > 0) {
					for(var sel = 0; sel < slotSelections.selections[coord].length; sel++) {
						...
						if(newSel.type != coordTypes[coord]) {
						...
						else if((coordTypes[coord] == 'string' && newSel.min >= coordLimits[coord].noofLabels) || (newSel.min > coordLimits[coord].max || newSel.max < coordLimits[coord].min)) {
							...
							if(coordTypes[coord] == 'string') {
								...
								if(newSel.min > coordLimits[coord].max || newSel.max < coordLimits[coord].min) {
									// outside of new range, skip
								}
								else {
				...
				for(sel = 0; sel < selections[coord].length; sel++) {
					...
					if(selections[coord][sel][4]) { // only null
						...
						if(coordTypes[coord] == 'string') {
							...
							if(selections[coord][sel][2] > coordLimits[coord].max || selections[coord][sel][3] < coordLimits[coord].min) {
								// outside of new range, skip
							}
							else {
							...
							if(coordTypes[coord] == 'string') {
								...
								if(s === null) {
									...
									if(coordLimits[coord].dict.hasOwnProperty(s)) {
										...
										for(var span = 0; span < selections[coord].length; span++) {
											if(!selections[coord][span][4] && selections[coord][span][2] <= idx && idx <= selections[coord][span][3]) {
												groupIdCoord = span + 1;
												break;
											}
										}
								...
								if(v === null) {
									...
									for(var span = 0; span < selections[coord].length; span++) {
										if(!selections[coord][span][4] && selections[coord][span][2] <= v && v <= selections[coord][span][3]) {
											groupIdCoord = span + 1;
											break;
										}
									}
	...
	function parseData() {
		...
		if(atLeastOneFilled) {
			...
			if(!dataIsCorrupt) {
				...
				coordNames.push(legacyDDSupLib.shortenName(name1));
				coordNames.push(legacyDDSupLib.shortenName(name2));
				...
				if(haveMore) {
					for(var more = 0; more < moreArrays.length; more++) {
						coordNames.push(legacyDDSupLib.shortenName(moreNames[more]));
					}
					for(var more = 0; more < moreArrays.length; more++) {
				...
				if(coordArrays.length != coordNames.length || coordArrays.length != coordTypes.length || coordArrays.length != coordLimits.length) {
					dataIsCorrupt = true;
				}
			...
			if(dataIsCorrupt) {
				...
				for(var coord = 0; coord < coordArrays.length; coord++) {
					...
					// $log.log(preDebugMsg +"layout coordinates, " + coord + " " + coordNames[coord] + " " + textW + " " + xpos);
	...
	function updateGraphicsHelper(forceBackground, forceLines, forceAxes) {
		...
		// Draw
		// ===========
		// $log.log(preDebugMsg +"Need to redraw: " + redrawBackground + " " + redrawLines + " " + " " + redrawAxes);
		if(redrawBackground) {
			drawBackground(W, H);
		}
		if(redrawAxes) {
	...
	// This method draws the coordinate bars.
	//===================================================================================
	...
	//===================================================================================
	// Draw Coordinate Bar Shadow
	// This method draws the coordinate bar shadows.
	//===================================================================================
	...
	// This method draws the coordinate labels.
	//===================================================================================
	...
	function drawPolyLines(W,H) {
		...
		if(unique > quickRenderThreshold) {
			drawPretty = false;
			var rgba0 = localHexColorToRGBAvec(legacyDDSupLib.getColorForGroup(0, colorPalette, currentColors), 0.33);
			...
			for(var set = 0; set < Ns.length; set++) {
				...
				for(var i = 0; i < Ns[set]; i++) {
					...
					try {
						...
						//$log.log(preDebugMsg +"this should not happen!!");
						console.dir(e);
						console.dir(votes);
						console.dir(y1);
						console.dir(y2);
						...
						if(votes[y1][y2].length > 0) {
							...
							if(ls.length == 1) { // only unselected had votes
								if(pass == 0) {
									...
									if(transparency >= 1) {
										var color = localHexColorToRGBAvec(legacyDDSupLib.getColorForGroup(groupId, colorPalette, currentColors), 0.33);
									}
									else {
										var color = localHexColorToRGBAvec(legacyDDSupLib.getColorForGroup(groupId, colorPalette, currentColors), transparency * 0.33);
									}
								...
								if(pass > 0) {
									...
									for(var groupId = 1; groupId < ls.length; groupId++) {
										if(ls[groupId] > 0) {
											if(noofGroups > 1) {
												var thisColor = localHexColorToRGBAvec(legacyDDSupLib.getColorForGroup(groupId, colorPalette, currentColors), transparency * ls[groupId] / noofVotes);
												...
												var color = localHexColorToRGBAvec(legacyDDSupLib.getColorForGroup(groupId, colorPalette, currentColors), transparency);
												draw = true;
											}
										}
									}
	...
	//===================================================================================
	// Draw Poly Lines Working
	// This method draws poly lines working.
	//===================================================================================
		...
		for(var pass = 0; pass < 2; pass++) {
			for(var set = 0; set < Ns.length; set++) {
				...
				for(var i = 0; i < Ns[set]; i++) {
					...
					if((groupId == 0 && pass == 0) || (groupId != 0 && pass != 0)) {
						if(drawPretty) {
							lineCtx.save();
							var color = legacyDDSupLib.getColorForGroup(groupId, colorPalette, currentColors);
							if(transparency >= 1) {
								if(pass == 0) {
									color = localHexColorToRGBA(color, 0.33);
								}
							}
							else {
								if(pass == 0) {
									color = localHexColorToRGBA(color, transparency * 0.33);
								}
								else {
									color = localHexColorToRGBA(color, transparency);
								}
							}
							...
							if(pass > 0) {
								if(transparency >= 1) {
									var col = localHexColorToRGBAvec(legacyDDSupLib.getColorForGroup(groupId, colorPalette, currentColors), 1);
								}
								else {
									var col = localHexColorToRGBAvec(legacyDDSupLib.getColorForGroup(groupId, colorPalette, currentColors), transparency);
								}
							}
							else {
								var col = rgba0;
			...
			if(onlyNull) {
				...
				if(coordTypes[coord] == 'string') {
					...
					for(var i = 0; i < coordLimits[coord].props.length; i++) {
						if(prop1 <= coordLimits[coord].props[i] && coordLimits[coord].props[i] <= prop2) {
							if(!somethingInside) {
			...
			if(newSel != []) {
				...
				if(keepOld) {
					for(var s = 0; s < selections[coord].length; s++) {
						...
						if(sel[2] == newSel[2] && sel[3] == newSel[3] && sel[4] == newSel[4] && sel[5] == newSel[5]) {
	...
	//===================================================================================
	// Select All One Coordinate
	// This method selects all data points in one coordinate.
	//===================================================================================
	...
	function mousePosIsInSelectableArea(pos) {
		if(pos.x > leftMarg - 5 && pos.x <= leftMarg + drawW + 5 && pos.y > topMarg - 5 && pos.y <= topMarg + drawH + nullMarg + 5) {
			return true;
		}
		return false;
	}
	...
	//===================================================================================
	// Mouse Position in Header Area
	// This method checks whether the mouse pointer is within the header area.
	//===================================================================================
	function mousePosIsInHeaderArea(pos) {
		if(pos.x > leftMarg - 5 && pos.x <= leftMarg + drawW + 5 && pos.y < topMarg && pos.y > topTopMarg) {
			return true;
		}
		return false;
	}
	...
	//===================================================================================
	// Mouse Position in Just Above Header Area
	// This method checks whether the mouse pointer is just above header area.
	//===================================================================================
	function mousePosIsJustAboveHeaderArea(pos) {
		if(pos.x > leftMarg - 5 && pos.x <= leftMarg + drawW + 5 && pos.y > 0 && pos.y < topTopMarg) {
			return true;
		}
		return false;
	};
	...
	//===================================================================================
	// Pixel to Value Display
	// This method converts a pixel to a value for displaying.
	//===================================================================================
	...
	//===================================================================================
	// Pixel to Value Select
	// This method converts a pixel to a value for selection.
	//===================================================================================
	...
	//===================================================================================
	// Value to Pixel
	// This method converts a value to a pixel.
	//===================================================================================
	...
	//===================================================================================
	// Pixel to Nearest Header
	// This method returns the nearest header based on a specified pixel.
	//===================================================================================
	...
	//===================================================================================
	// Pixel to Nearest Coordinate
	// This method returns the nearest coordinate based on the specified pixel.
	//===================================================================================
	...
	//===================================================================================
	// Update Selection Display Size
	// This method updates the selection display size.
	//===================================================================================
	...
	//===================================================================================
	// Move Coordinate
	// This method manages the moving of a specified parallel coordinate.
	//===================================================================================
	function moveCoord(coord, x) {
		...
		if(coordXpos[coord] != x) {
			...
			for(coord = 0; coord < coordArrays.length; coord++) {
				...
				if(oldC >= logarithmic.length || logarithmic[oldC] != displayList[coord]) {
					dirty = true;
				}
			}
	...
	//===================================================================================
	// Mouse Up or Out
	// This method checks whether the mouse button is up or outside the allowed area.
	//===================================================================================
