...
wblwrld3App.controller('stepCurvePluginWebbleCtrl', function($scope, $log, Slot, Enum) {
	...
	for(var set = 0; set < Ns.length; set++) {
		...
		if(dataArray[i] === null) {
			...
			for(var span = 0; span < selections.length; span++) {
			    if(selections[span][0] <= dataArray[i] 				   
	...
	if(unique > 0) {
		...
		newSel[2] = xPos1;
		if(useLogX) {
		    // newSel[0] = Math.log(1 + (xPos1 - leftMarg)) / Math.log(1 + drawW) * (limits.max - limits.min) + limits.min;
		...
		if(useLogX) {
		    // newSel[1] = Math.log(1 + (xPos2 - leftMarg)) / Math.log(1 + drawW) * (limits.max - limits.min) + limits.min;
	...
	if(unique > 0) {
		...
		if(mousePosIsInSelectableArea(currentMouse)) {
			...
			if(idx < 0) {
				...
				s += '<span style="color: ' + getColorForGroup(groups[g]) + '">' + groupBuckets[groups[g]][idx] + '</span>';
			...
			if(x >= 0 && x < stepCurveCounts.length) {
				...
				if(stepCurveCounts[x].hasOwnProperty(groups[g])) {
				    ...
				    s += '<span style="color: ' + getColorForGroup(groups[g]) + '">' + stepCurveCounts[x][groups[g]] + '</span>';
