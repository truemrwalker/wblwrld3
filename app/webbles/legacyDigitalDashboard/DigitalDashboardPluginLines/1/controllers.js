//======================================================================================================================
// Controllers for DigitalDashboardPluginLinePlots for Webble World v3.0 (2013)
...
wblwrld3App.controller('linePlotPluginWebbleCtrl', function($scope, $log, Slot, Enum) {
	...
	if($scope.doDebugLogging) {
	    $log.log("DigitalDashboard Line Plot: " + message);
	}
    }
	...
	if(unique > quickRenderThreshold) {
		...
		if((xArray[i] < zoomMinX && xArray[j] < zoomMinX)
	...
	for(var pass = 0; pass < 3; pass++) {
		...
		if(set < globalSelections.length) {
		    ...
		    if((yArray[i] < zoomMinY && yArray[j] < zoomMinY)
		       || (yArray[i] > zoomMaxY && yArray[j] > zoomMaxY)) {
			skipY = true; // line completely outside view
		    } // this could be done better, check if the line intersects the rectangle
		    if((y2Array[i] < zoomMinY && y2Array[j] < zoomMinY)
		       || (y2Array[i] > zoomMaxY && y2Array[j] > zoomMaxY)) {
			skipY2 = true; // line completely outside view
		    } // this could be done better, check if the line intersects the rectangle

		    var diffI = y2Array[i] - yArray[i];
		    var diffJ = y2Array[j] - yArray[j];
		    if((diffI < zoomMinY && diffJ < zoomMinY)
		       || (diffI > zoomMaxY && diffJ > zoomMaxY)) {
