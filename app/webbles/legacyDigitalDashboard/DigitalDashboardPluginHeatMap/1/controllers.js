//======================================================================================================================
// Controllers for DigitalDashboardPluginHeatMap Webble World v3.0 (2013)
...
wblwrld3App.controller('heatMapPluginWebbleCtrl', function($scope, $log, Slot, Enum, $location) {
    ...
    function debugLog(message) {
    	if($scope.doDebugLogging) {
    	    $log.log("DigitalDashboard Heat Map: " + message);
    	}
    }
	...
	// debugLog("doBackgroundRowClustering");
	...
	debugLog("rowClusteringFinished");
	var data = e.data;
	...
	// debugLog("clusterRowsClustering");
	...
	// debugLog("doBackgroundColClustering");
	...
	debugLog("colClusteringFinished");
	var data = e.data;
	...
	// debugLog("clusterColsClustering");
	...
	var dirty = false;
	for(var r = 0; r < noofRows; r++) {
	    // debugLog("row " + r + " new=" + rowSelectedState[r] +" old="+ rowSelectedStateOld[r]);
    ...
    function parseData() {
    	...
    	if(parentInput.length > 0) {
			...
			dragZoneVecs.name = rowDataName;
    			...
    			debugLog("No column name metadata!!");
    		    }
    		}
		...
		for(i = 0; i < noofCols; i++) {
    		...
    		for(var source = 0; source < sources; source++) {
    			...
    			if(columnIdArray.length != columnArray.length) {
    			    ...
    			    debugLog("Column Array is not the same length as the column name metadata.");
    			    dataIsCorrupt = true;
    			}
    		    }
    		}
	...
	if(lastRowDataName != rowDataName
	...
	var colorScaleChange = false;
    	if(heatLimits.min < 0
    	   && heatLimits.max > 0) {
    	    if(hotColor != lastHotColor || 
	       zeroColor != lastZeroColor || 
	       coldColor != lastColdColor) {
		colorScaleChange = true;
	    }
    	} else if(heatLimits.min < 0) {
    	    if(zeroColor != lastZeroColor || 
	       coldColor != lastColdColor) {
		colorScaleChange = true;
	    }
    	} else {
	...
	if(selectionBarW != lastSelectionBarW
	   || selectionBarLeftMargin != lastSelectionBarLeftMargin
	   || barH != lastBarH
	...
	if(cellW != lastCellW
	   || cellH != lastCellH
	   || showBorder != lastShowBorder
	   || (showBorder && borderColor != lastBorderColor))
	{
	    redrawHeatmap = true;
	}
	...
	if(cluster[1].length >= 2) { // we have children
	    ...
	    // line to first child
	    //debugLog("cluster[1]=" + JSON.stringify(cluster[1]));
	...
	// debugLog("drawSelectionBar " + drawH);
	if(barCanvas === null) {
	...
	barCtx.clearRect(0,0, barCanvas.width, barCanvas.height);
    	...
    	var grd = barCtx.createLinearGradient(X, topMarg + selectionBarTopMargin,
    					      X, topMarg + selectionBarTopMargin + barH);
    	if(heatLimits.min < 0
    	   && heatLimits.max > 0) {
	...
	var startTime = Date.now();
    	// debugLog("drawHeatMap() start " + startTime);
    ...
    function getColorForValue(v) {
	    ...
	    // var prop = Math.abs(v / (heatLimits.max - heatLimits.min));
	    ...
	    var col = "#" + rs + gs + bs;
	    // var col = "#" + Math.floor(prop * r).toString(16) + Math.floor(prop * g).toString(16) + Math.floor(prop * b).toString(16);
	    return col;
    	}
    };
	...
	// clusterCanvas.style.top = "0px";
	// clusterCanvas.style.left = "0px";
	...
	case "SortCols":
	    ...
	    break;
	// case "MakeTransactionsForAllRows":
	//     if(eventData.slotValue) {
	// 	if(!transactionsForAll) {
	// 	    transactionsForAll = true;
	// 	    buildTransactions();
	// 	}
	//     } else {
	// 	if(transactionsForAll) {
	// 	    transactionsForAll = false;
	// 	    buildTransactions();
	// 	}
	//     }	    
	//     break;
    ...
    function newSelection(y1,y2, keepOld) {
    	...
    	if(unique > 0) {
    		...
    		if(sel[2] == newSel[2]
    		   && sel[3] == newSel[3]
    		   && sel[sel.length - 1] == newSel[newSel.length - 1]) {
    ...
    function mousePosIsInSelectableArea(pos) {
    	if(pos.x > selectionBarLeftMargin - 5
    	   && pos.x <= selectionBarLeftMargin + selectionBarW + 5
    	   && pos.y > topMarg + selectionBarTopMargin - 5
    	   && pos.y <= topMarg + drawH - selectionBarBottomMargin + 5) {
    	    return true;
    	}
    	return false;
    };
	...
	// This can lead to loops (build new transactions, they are automatically sent to someone else, this someone selects on this, and we need to build new transactions, etc.
        // $scope.addSlot(new Slot('MakeTransactionsForAllRows',
    	// 			true,
    	// 			"Make Transactions for All Rows",
    	// 			'Should transactions be generated for all rows or only selected rows.',
    	// 			$scope.theWblMetadata['templateid'],
    	// 			undefined,
    	// 			undefined
    	// 		       ));

        // $scope.addSlot(new Slot('RemoveEmptyTransactions',
    	// 			false,
    	// 			"Remove Empty Transactions",
    	// 			'Should rows with no transactions be given empty transactions or should these be removed.',
    	// 			$scope.theWblMetadata['templateid'],
    	// 			undefined,
    	// 			undefined
    	// 		       ));
	...
	// stuff that we use to send data to the parent
        ...
        // $scope.addSlot(new Slot('TransactionIDs',
	// 			[],
	// 			'Transaction IDs',
	// 			'The IDs of the generated data.',
	// 			$scope.theWblMetadata['templateid'],
	// 			undefined,                                 
	// 			undefined
	// 		       ));
