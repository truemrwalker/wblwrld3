...
module.exports = (function() {
	...
	var config = {
        ...
        SERVER_PORT: 7443,
        SERVER_PORT_INSECURE: 7000,
        SERVER_PORT_PUBLIC: 7443,
		...
		//     MONGODB_URL, SERVER_URL, SERVER_URL_INSECURE, SERVER_URL_PUBLIC
    ...
    if (process.env.PORT)
        config.SERVER_PORT_INSECURE = parseInt(process.env.PORT, 10);
    ...
    if (!config.SERVER_BEHIND_REVERSE_PROXY) {
        ...
        if (config.SERVER_PORT == config.SERVER_PORT_INSECURE)
            throw new EvalError("Insecure port is same as secure port");
    }
    else {
        
        config.SERVER_PROTO = "http";
    ...
    config.SERVER_URL = config.SERVER_PROTO + "://" + config.SERVER_NAME + 
        (config.SERVER_PORT == 443 ? '' : ':' + config.SERVER_PORT);

    checkPort(config.SERVER_PORT_INSECURE);
    config.SERVER_URL_INSECURE = "http://" + config.SERVER_NAME +
        (config.SERVER_PORT_INSECURE == 80 ? '' : ':' + config.SERVER_PORT_INSECURE);
